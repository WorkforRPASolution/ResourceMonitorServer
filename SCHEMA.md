...
>
> **비활성 rule(`enabled:false`)도 동일 검증(엄격)**: 위 1~8은 rule의 `enabled` 여부와 무관하게 적용됩니다 — 끈 상태로 깨진 참조를 저장할 수 없으므로 다시 켜는 순간 항상 안전합니다.
...
- **interval(cadence)도 임계/value처럼 모든 scope에서 override·도입 가능.** 스케줄러는 `(process, interval)` job을 등록하되, 그 interval 집합을 **전역·공정·모델·장비 모든 scope 문서**에서 수집(`get_scheduling_intervals`)하므로 model/eqp overlay가 도입한 새 interval도 스케줄·평가된다. (과거 process 레벨로 제한하던 `_check_interval_scope` 가드는 제거됨 — `docs/rms-profile-registration-rules-decision-2026-06-19.md`)
