...
- 출처 추적(provenance)·effective-profile 조회 API는 **구현 완료**(`GET /profiles/effective?withProvenance=1` — 각 항목에 기여 scope 라벨). 상속 항목 **삭제**(tombstone)는 추후 도입(현재는 overlay에서 통째 교체만).
...
- **schedule/interval override는 process 레벨까지만.** 스케줄러는 process 레벨 effective의 interval 집합으로 `(process, interval)` job을 등록하므로, model/eqp overlay가 **process에 없는 새 interval을 도입하면 API가 422로 거부**합니다(`_check_interval_scope`) — 그러지 않으면 그 rule이 스케줄되지 않아 silent lost breach. 임계/value override는 eqp 레벨 가능.
...
| 영역 | 구현 | 비고 |
|------|------|------|
| `src/db/models.py` | ✅ `MonitorProfile(scope+measures+rules+notify+enabled+governance)` + 문서/effective 검증(`validate_effective`) + lint 경고(`lint_effective`) | Pydantic 단일 진실 소스 |
