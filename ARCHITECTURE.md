# ARCHITECTURE — ResourceMonitorServer Phase 0
...
2. job은 ZK 분산 락을 잡고 ES에 시계열 쿼리 → 통계/임계값 비교 (Phase 1+)
3. 이상 발견 → Redis cooldown 확인 → Akka Email API 호출
4. 메트릭/로그/이벤트는 Prometheus + structlog JSON으로 표출

Phase 0에서는 위 흐름 중 **2단계의 락/쿼리/스케줄 골격**, **3단계의 cooldown/email 클라이언트**까지 구현 완료. 실제 분석 로직은 Phase 1.
...
- `client.py` — `AsyncElasticsearch` 래퍼. `ping()`, `introspect_field_type()`, NotFoundError → "unknown" 캐싱.
...
- `models.py` — Pydantic v2 기반 `Scope`, `MonitorProfile`. `extra="forbid"`, `populate_by_name=True`. `Scope.eqp_model = Field(alias="model")` (Mongo 필드명은 `model`).
- `repository.py` — `ProfileRepository` (TTLCache 내장), `EqpInfoRepository` (`{onoff: 1, webmanagerUse: 1}` 활성 필터 자동 적용).
...
- `email_client.py` — `_SUCCESS_RESULT = "success"` (lowercase). 5가지 예외 분기.
...
### `src/scheduler/`
- `jobs.py` — `AnalysisScheduler`. `AsyncIOScheduler` 래핑. **자체 `_running` 플래그**(APScheduler `running` 속성은 `shutdown(wait=False)` 후에도 True로 남는 버그 회피).
...
- `metrics.py` — Prometheus collectors (`JOB_TOTAL`, `JOB_DURATION`, `ES_QUERY_DURATION`, `ALERTS_SENT`, `ZK_LEADER`, `ASSIGNED_PROCESSES`).
...
### G8. Akka Email API `result` 는 lowercase

```python
# ❌ Akka 응답: {"result": "success", ...} — 대문자 아님
return data.get("result") == "Success"

# ✅
_SUCCESS_RESULT = "success"
return data.get("result") == _SUCCESS_RESULT
```

`tests/unit/test_email_client.py::test_returns_false_on_capital_success` 가 회귀 가드.
...
| Phase 1 | 임계값 분석 로직 + 실제 알림 발송 + `AnalysisScheduler.reload()` 에서 `resolve_processes_for_debug()` 연결 | 미착수 |
...
- DB 스키마 레퍼런스: [SCHEMA.md](SCHEMA.md)
