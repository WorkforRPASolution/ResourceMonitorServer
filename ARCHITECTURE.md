...
### `src/scheduler/`
- `jobs.py` — `AnalysisScheduler`. `AsyncIOScheduler` 래핑. `reload(processes)` 가 process 별 `get_scheduling_intervals(process)`(해당 process 에 영향 주는 모든 scope 문서의 **활성** rule `interval_minutes` 합집합 — model/eqp overlay 포함)로 distinct interval 별 job 등록(id=`analysis-{process}-{interval}m`). interval은 rule 소유(measure는 window만); 엔진이 런타임에 장비별 effective를 재해석하므로 per-eqp override는 그대로 적용됨. `_engine` lazy 초기화로 `AnalysisEngine` 연동.
  - **cadence reconcile** — 프로파일 편집으로 평가주기(interval) 집합이 바뀌면 **pod 재시작·파티션 재배치 없이** 반영. `reconcile()` 이 소유 process 의 interval 을 Mongo 에서 재계산해 등록 job 과의 **차이(delta)만** add/remove 한다(`remove_all_jobs` 안 함 → 미변경 job 의 `next_run` 보존; 이 set-diff 자체가 "주기 변경 감지"). 트리거 3가지: ① 프로파일 쓰기 직후 자동(best-effort — `profiles.py` 의 모든 쓰기 핸들러) ② 주기 루프(`MONITOR_SCHEDULER_RECONCILE_INTERVAL_SEC` 초마다, 기본 60·0이면 비활성) ③ `POST /admin/scheduler/reload`. `reload()`/`reconcile()` 은 `_reconcile_lock` 으로 직렬화, paused/shutdown 중엔 no-op. **debug 모드는 주기 루프 미기동**(수동/쓰기 트리거만 — "수동 관찰자" 계약 보존). 내용(임계값·notify 등) 변경은 엔진이 매 tick Mongo 재조회로 이미 자동 반영되므로 reconcile 불필요(cadence 변경만 담당).
...
- `admin.py` — `/admin/status`, `DELETE /admin/cooldowns` (query param `process,eqp_id,proc,notify,severity` — 5-dim identity), `POST /admin/scheduler/reload` (소유 process 즉시 cadence reconcile → `{reconciled}` 반환; Mongo 다운=503), `GET /admin/email-outbox`.
...
```
PartitionManager._apply_assignment(processes)        프로파일 편집(쓰기/주기/admin)
        │                                                    │
        ▼                                                    ▼
AnalysisScheduler.reload(processes)              AnalysisScheduler.reconcile()
   파티션 재배치 시 전체 재구성                       소유 process delta만 (재시작 불필요)
        │                                                    │
        │   (둘 다 _reconcile_lock 으로 직렬화)               │
        ├─ for process in processes:  ◄──────────────────────┘
        │      get_scheduling_intervals(process)   # 활성 rule interval 합집합(overlay 포함)
        │      for interval in <distinct intervals>:
