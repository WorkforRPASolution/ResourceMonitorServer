...
> 🟡 **v2 설계**: 문서 1개 = scope 1개. 내부는 3계층 — `measures[]`(잰다: category/metric/proc/window/facts), `rules[]`(판단: `measureId.type` 참조 + interval + severity + when + `enabled`), `notify{}`(알린다: cooldown/email_code). 단순 임계값은 조건 1개짜리 rule, 복합 조건은 조건 여러 개짜리 rule로 **통합** 표현한다. rule마다 `enabled`(기본 true)로 개별 on/off가 가능하다(문서 레벨 `enabled`와 별개; overlay에서 상속 rule을 끄는 소프트 tombstone). **전체 필드 레퍼런스·type 카탈로그·검증 규칙·예시는 [SCHEMA.md §1.4](SCHEMA.md)** 가 권위 문서다.
>
> ℹ️ 아래 §5.2/§5.3 v1 JSON에 보이는 `enabled` 필드는 **v1의 per-metric/per-analysis 플래그**(보존용)이며, v2의 `rule.enabled`(rule 단위)와는 다른 개념이다.
>
...
> ⚠️ **v3 재정정**: 실제 운영 EARS row의 문자열 필드(`EARS_EQPID`/`EARS_METRIC`/`EARS_CATEGORY`/`EARS_PROCNAME`/`EARS_MODEL` 등)는 전부 **`text` + `.keyword` 서브필드**(ES 기본 동적 매핑)로 매핑돼 있다(운영 인덱스 매핑으로 확인). term 필터·terms 집계는 **반드시 `<field>.keyword`** 를 써야 한다(bare text는 집계 400 실패·필터 토큰 mismatch). suffix는 `settings.es_keyword_suffix`(기본 `.keyword`)로 주입하며, bare keyword 클러스터면 `""`로 override([SCHEMA.md §8.1](SCHEMA.md), `src/es/queries.py`의 `keyword_field`). `EARS_VALUE`(numeric)·`EARS_TIMESTAMP`(date)는 bare. (앞선 "v2 정정"의 *bare keyword* 단정은 오류였음.)
...
```json
{
  ...
  "aggs": {
    "by_eqp": {
      "terms": { "field": "EARS_EQPID.keyword", "size": 20000 },
      "aggs": {
...
```json
{
  ...
  "aggs": {
    "by_model": {
      "terms": { "field": "EARS_MODEL.keyword" },
      "aggs": {
        "stats": { "extended_stats": { "field": "EARS_VALUE" } },
