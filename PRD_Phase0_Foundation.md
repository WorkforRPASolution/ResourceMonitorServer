...
기존 EARS DB(MongoDB)에 2개의 새 컬렉션을 추가한다.
...
**우선순위**: eqpId 지정 > model 지정 > process 지정 > 전체 기본값

특정 장비에 적용할 프로파일을 결정할 때, 가장 구체적인 scope가 우선한다. 예를 들어 장비 EQP001(process=PHOTO, model=MODEL_A)에 대해:

1. `scope: { process: "PHOTO", model: "MODEL_A", eqpId: "EQP001" }` — 있으면 최우선
2. `scope: { process: "PHOTO", model: "MODEL_A", eqpId: "*" }` — 없으면 model 레벨
3. `scope: { process: "PHOTO", model: "*", eqpId: "*" }` — 없으면 process 레벨
4. `scope: { process: "*", model: "*", eqpId: "*" }` — 없으면 전체 기본값

### 5.2 RESOURCE_MONITOR_PROFILE

메트릭별 모니터링 설정을 관리한다. Phase 1~2에서 주로 사용된다.
...
### 5.3 RESOURCE_MONITOR_RULE (Phase 3)

복합 조건 판단 규칙을 관리한다. Phase 3에서 구현하지만, 스키마는 Phase 0에서 확정한다.
